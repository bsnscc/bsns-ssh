...
/* mosh's Display::Display lived in terminaldisplayinit.cc (terminfo/setupterm),
   which we drop. SwiftTerm is xterm-class, so advertise ECH + BCE; we drive it
   directly, with no alternate screen and no title updates. Everything below is
   mosh 1.4.0's real diff-based renderer, verbatim -- it replaces the previous
   full-repaint stub that produced garbled rows on busy TUIs. */
Display::Display( bool /*use_environment*/ )
  : has_ech( true ), has_bce( true ), has_title( false ), smcup( 0 ), rmcup( 0 )
{}
