...
open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate, TerminalDelegate, UIPointerInteractionDelegate {
    ...
    func setupGestures ()
    {
        ...
        // bsns fork: desktop-class selection. A pointer (trackpad / mouse) click-drag
        // selects text; the scroll view's own pan is restricted to direct (finger)
        // touches so click-drag no longer scrolls. Finger long-press selection and
        // two-finger / finger scrolling are unchanged.
        enablePointerSelection()
        panGestureRecognizer.allowedTouchTypes = [NSNumber(value: UITouch.TouchType.direct.rawValue)]
    }

    // bsns fork: pointer-only drag-to-select.
    var pointerSelectGesture: UIPanGestureRecognizer?
    func enablePointerSelection () {
        guard pointerSelectGesture == nil else { return }
        let g = UIPanGestureRecognizer (target: self, action: #selector(pointerSelectHandler))
        g.allowedTouchTypes = [NSNumber(value: UITouch.TouchType.indirectPointer.rawValue)]
        addGestureRecognizer(g)
        pointerSelectGesture = g
    }

    @objc func pointerSelectHandler (_ gestureRecognizer: UIPanGestureRecognizer) {
        let hit = calculateTapHit(gesture: gestureRecognizer).grid
        switch gestureRecognizer.state {
        case .began:
            let _ = becomeFirstResponder()
        ...
        for press in presses {
            ...
            if !kittyFlags.isEmpty {
                ...
                if let functionKey = kittyFunctionalKey(for: key.keyCode) {
                    ...
                    // bsns fork: only Shift+PageUp/Down scrolls the local buffer; a
                    // plain PageUp/Down falls through to be sent to the remote.
                    if (functionKey == .pageUp || functionKey == .pageDown) && key.modifierFlags.contains([.shift]) {
            ...
            case .keyboardPageUp:
                // bsns fork: forward PageUp to the remote (ESC[5~) so tmux copy-mode
                // and full-screen apps page; Shift+PageUp scrolls the local buffer.
